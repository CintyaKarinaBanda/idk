    ...
    try:
        ...
        with pd.ExcelWriter(archivo, engine='openpyxl') as writer:
            ...
            if not resumen.empty:
                ws2.cell(row=9, column=2, value="RESUMEN POR CUENTA")
                ws2.cell(row=9, column=2).font = Font(bold=True, size=14)
                ws2.merge_cells(start_row=9, start_column=2, end_row=9, end_column=9)
                ...
                for j, header in enumerate(headers):
                    ws2.cell(row=11, column=2+j, value=header)
                
                for i, row in enumerate(resumen.itertuples(index=False)):
                    for j, val in enumerate(row):
                        ws2.cell(row=12+i, column=2+j, value=val)
                
                aplicar_formato(ws2, [11, 11+len(resumen)], [2, 2+len(headers)-1])
            
            # Resumen por servicio
            if not resumen_servicio.empty:
                fila_inicio = 13 + len(resumen) if not resumen.empty else 9
                ...
                if criticas + warnings + info > 0:
                    try:
                        ...
                        ws2.cell(row=14+len(resumen), column=2, value="Estado")
                        ws2.cell(row=14+len(resumen), column=3, value="Cantidad")
                        
                        for i, (label, value) in enumerate(chart_data):
                            ws2.cell(row=15+len(resumen)+i, column=2, value=label)
                            ws2.cell(row=15+len(resumen)+i, column=3, value=value)
                        
                        aplicar_formato(ws2, [14+len(resumen), 14+len(resumen)+len(chart_data)], [2, 3])
                        
                        chart = PieChart()
                        chart.title = "Distribución de Alertas"
                        chart.height, chart.width = EXCEL_STYLES["CHART_HEIGHT"], EXCEL_STYLES["CHART_WIDTH"]
                        
                        data_ref = Reference(ws2, min_col=3, max_col=3, min_row=15+len(resumen), max_row=14+len(resumen)+len(chart_data))
                        labels_ref = Reference(ws2, min_col=2, min_row=15+len(resumen), max_row=14+len(resumen)+len(chart_data))
                        ...
                        ws2.add_chart(chart, f"E{18+len(resumen)+len(chart_data)}")
                    except: pass
